...
//    @ResponseBody
	public ResponseEntity<?> getUserRecipePicture(HttpServletResponse resp, @PathVariable String id) {
...
//	        return photoAttachment.getPhoto(resp, photo, fileName);
	...
	@GetMapping(path = "/admin/photo/{id}", produces = MimeTypeUtils.IMAGE_JPEG_VALUE)
//  @ResponseBody
	public ResponseEntity<?> getAdminRecipePicture(HttpServletResponse resp, @PathVariable String id) {
		int recipeId = Integer.valueOf(id);
		Optional<AdminRecipeBean> recipe = aRecipeService.findById(recipeId);
		if (recipe.isPresent()) {
			byte[] photo = recipe.get().getPhoto();
//	        String fileName = "recipe_"+recipeId+".jpg";
//	        photoAttachment.uploadToCloudinary(photo, fileName);				
